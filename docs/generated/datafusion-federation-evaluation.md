...
**Snowflake: the same ownership split is plausible through ADBC**

An owned Snowflake connector can reuse an existing ADBC driver beneath core federation. The current ADBC driver catalog lists Snowflake, and the Rust driver manager can load driver shared libraries. The Snowflake driver provides SQL execution, table/catalog metadata, authentication options, and Arrow result mappings. This makes a small Semantic DB adapter a credible alternative to adopting table-providers. It does not require us to implement Snowflake's wire protocol. [ADBC driver catalog](https://arrow.apache.org/adbc/24/driver/index.html), [Rust driver manager](https://arrow.apache.org/adbc/24/rust/driver_manager.html), [Snowflake driver](https://adbc-drivers.org/drivers/snowflake/).

Compared with ClickHouse's direct Rust-client/ArrowStream route, plan for more integration plumbing: driver binary distribution, Rust Arrow dependency alignment, running synchronous driver operations away from async executor threads, and propagating cancellation into the driver. Our adapter would own a bounded Arrow stream bridge, scan fallback, SQL capability policy, and an execution identity scoped to the resolved connection and its role/database/schema/warehouse settings. Numeric precision, timestamp variants, and semi-structured mappings require qualification. The driver exposes a high-precision numeric option, so we should choose and test its result contract explicitly. [Snowflake type and query options](https://adbc-drivers.org/drivers/snowflake/#feature-type-support).

The reviewed contrib ADBC factory returns a regular scan provider and its pool disallows join pushdown. Adopting that wrapper therefore does not currently give us ready-made Snowflake subplan federation. A direct `SQLExecutor` over ADBC can be a better place to integrate our connection identity and capability rules. My recommendation for ClickHouse plus Snowflake is core federation with small owned adapters over established clients/drivers, extracting shared adapter code only as concrete duplication appears. This remains an architectural recommendation; no Snowflake driver build or live query has been validated here. [Contrib ADBC factory](https://github.com/datafusion-contrib/datafusion-table-providers/blob/e5fdc19ae2c8b0d806a82c4eedbd4538dbbc59fc/crates/adbc/src/lib.rs), [contrib ADBC pool](https://github.com/datafusion-contrib/datafusion-table-providers/blob/e5fdc19ae2c8b0d806a82c4eedbd4538dbbc59fc/crates/adbc/src/pool.rs).
